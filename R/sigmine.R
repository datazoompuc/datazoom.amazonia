...
load_sigmine <- function(dataset = "sigmine_active",
  ...
  if (dataset == "sigmine_active") {
    ...
    if (geo_level == "municipality") {
      ...
      # sigmine_inactive has no ANM-reported "uf" to begin with, so the
      # shapefile's abbrev_state is simply added as "uf" for consistency
      # with sigmine_active's column naming.
  ...
  # The inactive/closed processes dataset published by ANM has far fewer
  # columns than the active one: only processo, numero, ano, area_ha, id,
  # ds_processo. There is no mineral, company, phase, last event, or state
  # (uf) at the source -- this is a limitation of what ANM publishes for
  # closed processes, not something this function can recover. In
  # particular, there is no closing date/year here either: "ano" is still
  # the year the process was originally opened, same caveat as in
  # sigmine_active.
  ...
  if (dataset == "sigmine_inactive") {
    ...
    if (geo_level == "municipality") {
      ...
      # Same projection choice and rationale as sigmine_active above.
      operation_crs <- sf::st_crs("+proj=aea +lat_1=-2 +lat_2=-22 +lat_0=-12 +lon_0=-54 +x_0=5000000 +y_0=10000000 +ellps=GRS80 +units=m +no_defs")
      ...
      # sigmine_inactive has no ANM-reported "uf" to begin with, so the
      # shapefile's abbrev_state is simply added, not used to overwrite
      # anything.
      a$uf <- a_munic$abbrev_state
    }
    ...
    if (language == "pt") {
      ...
      names(a)[names(a) == "uf"] <- "state"
      if (geo_level == "municipality") {
        names(a)[names(a) == "municipality_code"] <- "municipality_code"
        names(a)[names(a) == "municipality"] <- "municipality"
      }
    }
  }
