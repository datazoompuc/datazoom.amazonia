load_dictionary <- function(dataset) {
  ...
  if (stringr::str_detect(dataset, "datasus_sih_rd")) {
    harmonization_dat <- tibble::tribble(
      ...
      "dt_inter", "data_internacao",  "date_admission", "data da internação", "date of admission",
  ...
  if (stringr::str_detect(dataset, "datasus_sih_sp")) {
    harmonization_dat <- tibble::tribble(
      ...
      "abbrev_state", "abbrev_state",              "abbrev_state",           "Sigla do estado",                  "State abbreviation",
      "legal_amazon", "legal_amazon",              "legal_amazon",           "Amazonia Legal",                   "Legal Amazon",
      "sp_gestor",    "municipio_gestor",          "managing_municipality",  "Município gestor",                 "Managing municipality",
      "sp_uf",        "uf",                        "federal_unit",           "Unidade da Federação",             "Federative unit",
      "sp_aa",        "ano_internacao",            "year_admission",         "Ano da internação",                "Admission year",
      "sp_mm",        "mes_internacao",            "month_admission",        "Mês da internação",                "Admission month",
      "sp_cnes",      "cnes_estabelecimento",      "facility_cnes",          "CNES do estabelecimento",          "Facility CNES",
      "sp_naih",      "numero_aih",                "aih_number",             "Número da AIH",                    "AIH number",
      "sp_procrea",   "procedimento_realizado",    "procedure_performed",    "Procedimento realizado",           "Procedure performed",
      "sp_dtinter",   "data_internacao",           "admission_date",         "Data da internação",               "Admission date",
      "sp_dtsaida",   "data_saida",                "discharge_date",         "Data de saída",                    "Discharge date",
      ...
      "sp_m_hosp",    "municipio_estabelecimento", "facility_municipality",  "Município do estabelecimento",     "Facility municipality",
      "sp_m_pac",     "municipio_paciente",        "patient_municipality",   "Município do paciente",            "Patient municipality",
      ...
      "sp_pj_doc",    "doc_pj",                    "pj_doc",                 "Documento da pessoa jurídica",     "Legal entity document",
      "in_tp_val",    "tipo_valor",                "value_type",             "Tipo de valor",                    "Value type",
      "sequencia",    "codigo_sequencial",         "sequential_code",        "Código sequencial",                "Sequential code",
      "remessa",      "nome_remessa",              "batch_name",             "Nome da remessa",                  "Remittance name",
      "serv_cla",     "servico_classificacao",     "service_classification", "Serviço / Classificação",          "Service / Classification",
      "sp_cidpri",    "cid_principal",             "main_cid",               "CID principal",                    "Main ICD",
      "sp_cidsec",    "cid_secundario",            "secondary_cid",          "CID secundário",                   "Secondary ICD",
      "sp_qt_proc",   "quantidade_procedimentos",  "procedures_quantity",    "Quantidade de procedimentos",      "Number of procedures",
      "sp_u_aih",     "indicador_unico",           "unique_aih",             "Indicador único da AIH",           "Unique AIH indicator"
