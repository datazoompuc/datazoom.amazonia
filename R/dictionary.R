load_dictionary <- function(dataset) {
  ...
  if (stringr::str_detect(dataset, "datasus_sinasc")) {
    harmonization_dat <- tibble::tribble(
      ...
      "tpapresent", "tipo_apresentacao_recem_nascido", "newborn_presentation_type", "Tipo de apresentacão do RN", "Type of presentation of the newborn",
      ...
      "codmunnasc","municipio_nascimento","birth_municipality", "Codigo do município de nascimento", "Code of the municipality of birth",
      "seriescmae","serie_escolar_cmae","mother's_schooling_series", "Serie escolar da mae", "Mother's school series",
      "codocupmae","codigo_ocupacao_mae","mother_occupation_code", "Codigo de ocupacao da mae conforme tabela do CBO (Código Brasileiro de Ocupacoes)","Mother's occupation code according to the CBO (Brazilian Occupation Code) table",
      ...
      "gestacao", "semanas_gestacao_agrupado", "grouped_gestational_weeks", "Semanas de gestacão agrupado", "Weeks of gestation grouped",
      ...
      "codpaisres", "codigo_pais_residencia", "country_of_residence_code", "Codigo do pais de residência", "Country of residence code",
      ...
      "difdata", "dif_data_obito_recebimento", "death_to_receipt_date_diff", "Diferenca entre a data de obito e data do recebimento original da DO ([dtnasc] –[dtrecorig])", "Difference between the date of death and the date of original receipt of the DO ([dtnasc] – [dtrecorig])",
      ...
      "tpfuncresp","funcao_exercida_responsavel","function_performed_by_responsible_person", "Funcao exercida pelo responsável pelo preenchimento da declaracao", "Function performed by the person responsible for completing the declaration",
