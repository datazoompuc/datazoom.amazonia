...
load_degrad <- function(source, geo_level = "municipality",
                        time_period = "year",
                        language = "eng",
                        all_events = FALSE) {
  ...
  list_df <- lapply(raw_data, treat_degrad_data, geo_level = geo_level, time_period = time_period, language = language, geo_amazon = geo_amazon, filter = !all_events)
...
treat_degrad_data <- function(df, geo_level, time_period, language, geo_amazon, filter) {

  df = df %>% janitor::clean_names()

  message("Processing data. This should take a few minutes.")

  ## Select Only Degrad Data
