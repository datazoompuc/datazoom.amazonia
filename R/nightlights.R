...
#' @examples
#'
#  #ggplot2::ggplot() + ggplot2::geom_sf(data = sf::st_as_sf(n[n$year==2013,]),ggplot2::aes(fill = mean_rad))
#'
load_ee = function(db = 'NOAA/VIIRS/DNB/MONTHLY_V1/VCMSLCFG',var='avg_rad',year_begin = '2013',year_end = '2018',
  ...
  ext_fun = function(uf){
    ...
    nlight_avg = nlight %>% dplyr::group_by(id,year) %>%
      dplyr::summarise(mean_rad = mean(avg_rad),sd_avg_rad = sd(avg_rad),
                       geom=sf::st_union(geometry)) %>%
      dplyr::ungroup()
  ...
  if (nclusters == 0){nlight = suppressMessages(purrr::map_df(units,ext_fun))}
