...
Get current population distribution across states:

```{r eval=FALSE}
library(dplyr)

# Load 2021 population data at state level
...
state_ranking <- pop_2021 %>%
  ...
  mutate(
    rank = row_number(),
    pct_of_total = (population / sum(population)) * 100
  )

print(state_ranking)

# Concentration: Top 5 states account for what % of population?
...
Calculate how populations are changing over time:

```{r eval=FALSE}
library(dplyr)

# Load population for multiple years
...
# Average growth rate over period
avg_growth <- growth_analysis %>%
  group_by(state) %>%
  summarize(
    avg_annual_growth = mean(annual_growth, na.rm = TRUE),
