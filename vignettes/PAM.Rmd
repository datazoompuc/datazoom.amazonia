...
The package contains of three function: \code{load_pam_permanent}, \code{load_pam_temporary} and \code{load_pam_main} which extracts data from the API of the IBGE database website. It allows the user to input the years he would like the data to encompass, the geographical aggregation level desired, as well as choosing the language of the data (currently only Portuguese and English are supported). 

The output is a  dataframe containing data for the territorial level chosen, the name of such geogrpahic level, the year of the data, the variable in question, the unit of measurement and the value. Each column has a corresponding column containing the numerical code of the specific value the variable has, with the exception of the Value column, which is already numeric. 
...
data <- load_pam_permanent(2017, aggregation_level = "state", language = "eng")
