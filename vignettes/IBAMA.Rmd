...
```time_aggregation``` and ```space_aggregation``` set the levels through which the aggregation
...
To load previously downloaded raw data, set ```download_data = FALSE``` and specify the
file's path with the parameter ```load_from_where```. If ```language = 'pt```, use 
```space_aggregation = c('municipio', 'uf')``` and ```time_aggregation = c('ano', 'mes')``` instead:
