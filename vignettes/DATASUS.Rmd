...
Each original SIM data file contains rows corresponding to a death, and columns with several characteristics of the person, the place of death, and the cause of death. The data comes from 5 separate datasets: SIM-DO, SIM-DOFET, SIM-DOMAT, SIM-DOINF, and SIM-DOEXT, with corresponding dataset options `"datasus_sim_do", "datasus_sim_dofet", "datasus_sim_domat", "datasus_sim_doinf", "datasus_sim_doext"`. Using the option `dataset = "datasus_sim"`, data is pulled from all of the those datasets. Note that only SIM-DO provides separate files for each state, so all other dataset options always contain data from all states.
