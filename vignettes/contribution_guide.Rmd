...
## Contributing to datazoom.amazonia

Please follow this workflow when contributing to `datazoom.amazonia`:  
...
The point of the sections below is to present the structure of the `datazoom.amazonia` functions, and share our workflow of how to create new functions.
...
The other columns are also important. The first one shows the source of the data you are including in the package. The second one is the name of the dataset, which will be used in the **load_*****database*** function in case you have more than one dataset for this source. The column `sidra_code` is used only for datasets that come from IBGE and it corresponds to the ID of the dataset provided by [SIDRA-IBGE](https://sidra.ibge.gov.br/). 

Finally, the columns `available_time` and `available_geo` tell the user which years (2020, 2003-2020, 2010 & 2018, ...) and aggregations (Country, State, Municipality, ...) are available for that particular dataset.
...
Pretty much all of our functions follow the format below. Do not underestimate the utility of `#` comments and the big section separators such as for `## Bind Global Variables`. They make code a lot clearer. Add as many comments as possible, try to always explain what you're doing!
...
#' and once you fill it up with info, it creates the function help file
